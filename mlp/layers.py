...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        output = np.zeros((batch_size, self.num_output_channels, output_height, output_width))
        for batch in range(batch_size):
            for output_channel in range(self.num_output_channels):
                for input_channel in range(self.num_input_channels):
                    output[batch,output_channel]+=convolve2d(inputs[batch,input_channel],self.kernels[output_channel,input_channel], mode='valid')
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        batch_size=inputs.shape[0]
        bprop_val= np.zeros(inputs.shape)
        for batch in range(batch_size):
            for output_channel in range(self.num_output_channels):
                for input_channel in range(self.num_input_channels):
                    bprop_val[batch,input_channel]+=correlate2d(grads_wrt_outputs[batch,output_channel],self.kernels[output_channel,input_channel], mode='full')
        return bprop_val
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        for batch in range(batch_size):
            for output_channel in range(self.num_output_channels):
                for input_channel in range(self.num_input_channels):
                    grads_wrt_kernels[output_channel,input_channel] += correlate2d(grads_wrt_outputs[batch,output_channel],inputs[batch,input_channel], mode='valid')
                grads_wrt_biases[output_channel]+=np.sum(grads_wrt_outputs[batch,output_channel])
