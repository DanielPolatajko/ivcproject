"""Borrowed largely from MLP Coursework 2 support code, with some adjustments"""
...
class ExperimentBuilder(nn.Module):
    ...
    def run_train_iter(self, x, y):
        ...
        x, y = x.float().to(device=self.device), y.float().to(
        ...
        loss = F.binary_cross_entropy_with_logits(out, y)  # compute loss
    ...
    def run_experiment(self):
        ...
        total_losses = {"train_acc": [], "train_loss": [], "val_acc": [],
                        "val_loss": [],"train_jac": [], "val_jac" :[]}  # initialize a dict to keep the per-epoch metrics
        ...
        for i, epoch_idx in enumerate(range(self.starting_epoch, self.num_epochs)):
            ...
            current_epoch_losses = {"train_acc": [], "train_loss": [], "val_acc": [], "val_loss": [], "train_jac": [], "val_jac" :[]}
            ...
            with tqdm.tqdm(total=len(self.train_data)) as pbar_train:  # create a progress bar for training
                for idx in range(len(self.train_data)):  # get data batches
                    ...
                    current_epoch_losses["train_jac"].append(j)
                    pbar_train.update(1)
                    pbar_train.set_description("loss: {:.4f}, accuracy: {:.4f}, jaccard index : {:.4f}".format(loss, accuracy,j))
            ...
            with tqdm.tqdm(total=len(self.val_data)) as pbar_val:  # create a progress bar for validation
                for j in range(len(self.val_data)):  # get data batches
                    ...
                    #print(temp[0].shape)
                    x,y = torch.Tensor(temp[0]).to(device=self.device),torch.Tensor(temp[1]).to(device=self.device)
                    loss, accuracy, j = self.run_evaluation_iter(x=x, y=y)  # run a validation iter
                    current_epoch_losses["val_loss"].append(loss)  # add current iter loss to val loss list.
                    current_epoch_losses["val_acc"].append(accuracy)  # add current iter acc to val acc lst.
                    current_epoch_losses["val_jac"].append(j)
            ...
            storage_utils.save_statistics(experiment_log_dir=self.experiment_logs, filename=(self.experiment_name+'_summary.csv'),
        ...
        current_epoch_losses = {"test_acc": [], "test_loss": [], "test_jac": []}  # initialize a statistics dict
        with tqdm.tqdm(total=len(self.test_data)) as pbar_test:  # ini a progress bar
            for jdx in range(len(self.test_data)):  # sample batch
                temp = self.test_data.next()
                x,y = torch.Tensor(temp[0]).to(device=self.device),torch.Tensor(temp[1]).to(device=self.device)
                loss, accuracy, j = self.run_evaluation_iter(x=x,
                                                          y=y)  # compute loss and accuracy by running an evaluation step
                current_epoch_losses["test_loss"].append(loss)  # save test loss
                current_epoch_losses["test_acc"].append(accuracy)  # save test accuracy
                current_epoch_losses["test_jac"].append(j)  # save test accuracy
                pbar_test.update(1)  # update progress bar status
                pbar_test.set_description(
                    "loss: {:.4f}, accuracy: {:.4f}, jaccard index: {:.4f}".format(loss, accuracy,j))  # update progress bar string output

        test_losses = {key: [np.mean(value)] for key, value in
                       current_epoch_losses.items()}  # save test set metrics in dict format
        storage_utils.save_statistics(experiment_log_dir=self.experiment_logs, filename=(self.experiment_name+'_test_summary.csv'),
