...
class DenseProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(DenseProcessingBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['dense_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['dense_0'].forward(out)
        out = F.leaky_relu(out)

        self.layer_dict['dense_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['dense_1'].forward(out)
...
class DenseDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(DenseDimensionalityReductionBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['res_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['res_0'].forward(out)
        out = F.leaky_relu(out)

        out = F.avg_pool2d(out, self.reduction_factor)

        self.layer_dict['res_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['res_1'].forward(out)
        out = F.leaky_relu(out)
...
class ResidualProcessingBlock(nn.Module):
    ...
    def build_module(self):
        ...
        self.layer_dict['res_a'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        out = self.layer_dict['res_a'].forward(out)
        out = F.leaky_relu(out)
        self.layer_dict['res_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
...
class ResidualDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ResidualDimensionalityReductionBlock, self).__init__()
...
class ResidualBNProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ResidualBNProcessingBlock, self).__init__()
...
class ResidualBNDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ResidualBNDimensionalityReductionBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['resbn_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['resbn_0'].forward(out)
        out = F.leaky_relu(out)
        out = F.avg_pool2d(out, self.reduction_factor)
        self.layer_dict['resbn_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)

        out = self.layer_dict['resbn_1'].forward(out)
        out = F.leaky_relu(out)
...
class TrialNetwork(nn.Module):
    def __init__(self,input_shape, use_bias=False, processing_block_type=ConvolutionalProcessingBlock,
                 dimensionality_reduction_block_type=ConvolutionalDimensionalityReductionBlock):
    ...
    def build_module(self):
        ...
        out = x
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=3,out_channels=16,kernel_size=8)
        out = self.layer_dict['conv_0'].forward(out)
        out = F.leaky_relu(out)
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=16,out_channels=8,kernel_size=8)
        out = self.layer_dict['conv_1'].forward(out)
        out = F.leaky_relu(out)
        self.layer_dict['conv_T_2'] = nn.ConvTranspose2d(in_channels=8,out_channels=16,kernel_size=8)
        out = self.layer_dict['conv_T_2'].forward(out)
        out = F.leaky_relu(out)
        self.layer_dict['conv_T_3'] = nn.ConvTranspose2d(in_channels=16,out_channels=2,kernel_size=8)
        out = self.layer_dict['conv_T_3'].forward(out)
        #out = out.permute(0,2,3,1)
        print(out.shape)
        #self.lin = nn.Linear(in_features=out.shape[1],
                             #out_features=self.num_output_classes,
                             #bias=True)
        #out = self.lin.forward(out)  # apply linear layer on flattened inputs
        print("Block is built, output volume is", out.shape)
    ...
    def forward(self, x):
        ...
        out = self.layer_dict['conv_T_3'].forward(out)
        #out = out.permute(0,2,3,1)
        #out = self.lin.forward(out)  # pass through a linear layer to get logits/preds
        #print(out)
        return out
